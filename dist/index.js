...
const newline$1 = 22, newlineEmpty = 23, eof = 24, incomingNewline = 25, incomingEOF = 26, stafflineSep = 27;

const newline = 10, carriageReturn = 13, space = 32, tab = 9;
const newlines = new ExternalTokenizer((input) => {
    if (input.next < 0) {
...
const lookaheads = new ExternalTokenizer((input) => {
    if (input.next < 0) {
...
const stafflineSeparator = new ExternalTokenizer((input) => {
...
let parserWithMetadata = parser.configure({
    props: [
        styleTags({
            Fret: tags.integer,
            MeasureLineName: tags.name,
            Comment: tags.comment
        }),
    ],
});
const tablatureLanguage = LRLanguage.define({
